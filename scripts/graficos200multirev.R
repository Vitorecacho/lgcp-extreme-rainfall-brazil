...
load("/dados/extremerain/modeloextremerain200interceptMulti.Rdata")
...
par(mfrow=c(3,1))
ts.plot(resm2r$summary.random$Nrwtime[,4:6],col=c(2,1,2))
ts.plot(resm2r$summary.random$Nseastime[,4:6],col=c(2,1,2))
ts.plot(resm2r$summary.random$Nartime[,4:6],col=c(2,1,2))


par(mfrow=c(3,1))
ts.plot(resm2r$summary.random$NErwtime[,4:6],col=c(2,1,2))
ts.plot(resm2r$summary.random$NEseastime[,4:6],col=c(2,1,2))
ts.plot(resm2r$summary.random$NEartime[,4:6],col=c(2,1,2))


par(mfrow=c(3,1))
ts.plot(resm2r$summary.random$COrwtime[,4:6],col=c(2,1,2))
ts.plot(resm2r$summary.random$COseastime[,4:6],col=c(2,1,2))
ts.plot(resm2r$summary.random$COartime[,4:6],col=c(2,1,2))


par(mfrow=c(3,1))
ts.plot(resm2r$summary.random$SErwtime[,4:6],col=c(2,1,2))
ts.plot(resm2r$summary.random$SEseastime[,4:6],col=c(2,1,2))
ts.plot(resm2r$summary.random$SEartime[,4:6],col=c(2,1,2))


par(mfrow=c(3,1))
ts.plot(resm2r$summary.random$Srwtime[,4:6],col=c(2,1,2))
ts.plot(resm2r$summary.random$Sseastime[,4:6],col=c(2,1,2))
ts.plot(resm2r$summary.random$Sartime[,4:6],col=c(2,1,2))
...
make_component_df <- function(obj, comp, region){

  data.frame(

    time = dates[1:nrow(obj)],
...
df_all <- do.call(
  ...
  lapply(regions, function(r){
    ...
    rbind(
      ...
      make_component_df(
        resm2r$summary.random[[paste0(r,"rwtime")]],
        "Trend (RW2)",
        r
      ),

      make_component_df(
        resm2r$summary.random[[paste0(r,"seastime")]],
        "Seasonality",
        r
      ),

      make_component_df(
        resm2r$summary.random[[paste0(r,"artime")]],
        "Short-term (AR2)",
        r
      )
    )
...
df_all$region <- factor(
  df_all$region,
  levels = c("N","NE","CO","SE","S"),
  labels = c(
    "North",
    "Northeast",
    "Center-West",
...
p1 <- plot_component(
  df_all[df_all$component=="Trend (RW2)",]
)

p2 <- plot_component(
  df_all[df_all$component=="Seasonality",]
)

p3 <- plot_component(
  df_all[df_all$component=="Short-term (AR2)",]
)
...
df_list <- lapply(seq_along(t.mean), function(j) {
  data.frame(
    x = prj$lattice$loc[,1],
    y = prj$lattice$loc[,2],
    z = as.vector(t.mean[[j]]-resm2r$summary.fixed[1,1]),
    time = j,
    year = 1980 + j
  )
})
